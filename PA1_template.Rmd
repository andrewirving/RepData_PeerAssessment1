...
Mean daily steps is `r meanDailySteps`  
Median daily steps is `r medianDailySteps`
...
```{r plotAverageDailyPatterns, fig.width=10}
...
The 5-minute interval that contains on average the maximum number of steps is `r maxInterval` (`r maxIntervalTime`).  
...
In total there are `r cntStepsNAs` observations where the steps value is NA.  
...
After running this code there are now `r chkStepsNAs` obervations with NA for steps.  

```{r aggregateImputedData}
...
```{r calculateMeanAndMedianImputed}
...
Mean daily steps is `r meanDailyImputedSteps`  
Median daily steps is `r medianDailyImputedSteps`
...
```{r aggregateWeekdayAndWeekendProfiles}
...
```{r plotWeekdayWeekendComparison, fig.width=10}
library(lattice)
